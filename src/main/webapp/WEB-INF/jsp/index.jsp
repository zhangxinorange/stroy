    ...
    <div class="middle">
        ...
        <form action="${pageContext.request.contextPath}/list" method="post">
            <table class="gridtable" style="width:100%;">
                <tr>
                    <th>电话：</th>
                    <td><input type="text" name="mPhone" value="${queryParam.mPhone}"/></td>
                    <th>邮箱：</th>
                    <td><input type="text" name="mEmail" value="${queryParam.mEmail}"/></td>
                ...
                <c:forEach items="${pageInfo.list}" var="member">
                    <tr>
                        ...
                        <td style="text-align:center;">[<a
                                href="${pageContext.request.contextPath}/view?mId=${member.mId}">修改</a>] -
                            [<a href="${pageContext.request.contextPath}/delete?mId=${member.mId}">删除</a>]
                        </td>
                    </tr>
                </c:forEach>
                </tbody>
            ...
            <table class="gridtable" style="width:100%;text-align: center;">
                <tr>
                    <c:if test="${pageInfo.hasPreviousPage}">
                        <td>
                            <a href="${pageContext.request.contextPath}/list?page=${pageInfo.prePage}&rows=${pageInfo.pageSize}&mEmail=${queryParam.mEmail}&mPhone=${queryParam.mPhone}">前一页</a>
                        </td>
                    </c:if>
                    <c:forEach items="${pageInfo.navigatepageNums}" var="nav">
                        ...
                        <c:if test="${nav != pageInfo.pageNum}">
                            <td>
                                <a href="${pageContext.request.contextPath}/list?page=${nav}&rows=${pageInfo.pageSize}&mEmail=${queryParam.mEmail}&mPhone=${queryParam.mPhone}">${nav}</a>
                            </td>
                        </c:if>
                    </c:forEach>
                    <c:if test="${pageInfo.hasNextPage}">
                        <td>
                            <a href="${pageContext.request.contextPath}/list?page=${pageInfo.nextPage}&rows=${pageInfo.pageSize}&mEmail=${queryParam.mEmail}&mPhone=${queryParam.mPhone}">下一页</a>
                        </td>
                    </c:if>
                </tr>
            </table>
