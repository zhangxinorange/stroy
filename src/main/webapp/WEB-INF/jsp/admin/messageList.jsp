...
<body>
			...
			<div class="span3" id="sidebar">
				<ul class="nav nav-list bs-docs-sidenav nav-collapse collapse">
					...
					<li ><a href="${ctx }/news/newsList"><i class="icon-chevron-right"></i>
							新闻通知管理</a></li>
					<li class="active"><a href="${ctx }/news/messageList"><i class="icon-chevron-right"></i>
							文章评论</a></li>
				</ul>
			</div>
			<!--/span-->
				...
				<div class="row-fluid">
					...
					<div class="block">
						<div class="navbar navbar-inner block-header">
							<div class="muted pull-left">回复列表</div>
						</div>
						<div class="block-content collapse in">
							<div class="span12">
									...
									<%-- <div class="btn-group">
										<a
											href="javascript:window.location.href='${ctx }/news/addNews'"><button
												...
												内容回复 <i class="icon-plus icon-white"></i>
											</button></a>
									</div> --%>
								</div>
								<table cellpadding="0" cellspacing="0" border="0"
									...
									<tbody>
										<c:forEach items="${data.data}" var="msg">
											<tr class="gradeA">
												<td>${msg.cTitle}</td>
												<td>${msg.mName}</td>
												<td>
													<c:if test="${fn:length(msg.mContent)>30 }">  
								                         ${fn:substring(msg.mContent, 0, 30)}...  
								                   </c:if> <c:if test="${fn:length(msg.mContent)<=30 }">  
								                         ${msg.mContent }  
                  									 </c:if>
												</td>
												<td><fmt:formatDate value="${msg.mCreateDate}" pattern="yyyy-MM-dd" /></td>
												
												<td style="text-align: center;"> [<a
													href="javascript:deleteContent('${msg.mId}');">删除</a>]
												</td>
											</tr>
										</c:forEach>
								...
								<div class="row">
									...
									<div class="span6">
										<div class="dataTables_paginate paging_bootstrap pagination">
											<ul>
											<li class="prev 
												"><a href="${ctx }/news/messageList?page=${data.pageIndex-1 }">← 上一页</a></li>
												 <c:forEach  begin="1" end="${data.pageCount}" var="nav">
							                        ...
							                        <c:if test="${nav != pageInfo.pageIndex}">
							                        	<li ><a href="${ctx }/news/messageList?page=${nav}">${nav}</a></li>
							                        </c:if>
												</c:forEach>
												<li class="next"><a href="${ctx }/news/messageList?page=${pageInfo.pageIndex+1}">下一页 → </a></li>
											</ul>
										</div>
									</div>
								</div>
	...
	<script>
		...
		function deleteContent(id)
		{
			confirm("确认删除吗？")
			{
				window.location.href="${ctx}/news/deleteMessage?mId="+id;
			}
		}
	</script>
</body>
